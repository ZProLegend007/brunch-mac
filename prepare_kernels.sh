...
for kernel in $kernels; do
	...
	case "$kernel" in
		6.1.62)
			...
			mkdir "./kernels/chromebook-6.1.62" #"./kernels/6.1.62"
			tar -C "./kernels/chromebook-6.1.62" -zxf "./kernels/chromiumos-$kernel.tar.gz" || { echo "Kernel $kernel source extraction failed"; exit 1; }
#			tar -C "./kernels/6.1.62" -zxf "./kernels/chromiumos-$kernel.tar.gz" chromeos || { echo "Kernel $kernel source extraction failed"; exit 1; }
			rm -f "./kernels/chromiumos-$kernel.tar.gz"
			apply_patches "chromebook-6.1.62"
			make_config "chromebook-6.1.62"
...
#			tar -C "./kernels/6.1.62" -zxf "./kernels/mainline-$kernel.tar.gz" --strip 1 || { echo "Kernel $kernel source extraction failed"; exit 1; }
#			rm -f "./kernels/mainline-$kernel.tar.gz"
#			apply_patches "6.1.62"
#			make_config "6.1.62"
		;;
	esac
done
